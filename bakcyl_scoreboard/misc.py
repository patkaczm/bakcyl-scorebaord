...
def updateTasksForUser(user):
    ...
    if "data" in user_tasks:
        ...
        print("Data for {} not found".format(pi.codewars_name))
    print("Tasks for {} had beed updated.".format(user.username))


def updateCwTaskss():
    for pi in PersonalInfo.objects.all():
        user_tasks = requests.get(
            'https://www.codewars.com/api/v1/users/{}/code-challenges/completed'.format(pi.codewars_name))
        user_tasks = user_tasks.json()
        if "data" in user_tasks:
            user_tasks = user_tasks['data']
            ...
            for task in user_tasks:
                if "cpp" in task["completedLanguages"]:
                    if not user_cw_tasks.filter(cwId=task["id"]).exists():
                        ...
                        cwTask.cwId = task["id"]
                        cwTask.completedAt = task["completedAt"]
                        cwTask.kyu = -requests.get(
                            'https://www.codewars.com/api/v1/code-challenges/{}'.format(task['id'])).json()["rank"][
                            "id"]
                        cwTask.save()
        else:
            print("Data for {} not found".format(pi.codewars_name))
